...
export default function DealInvoices({ deal, user }: DealInvoicesProps) {
  ...
  // If viewing an invoice, show the full-screen overlay
  if (viewingInvoice) {
    // FIX: Inject specific invoice details into the snapshot
    const snapshotDeal = {
      ...deal,
      // Pass the specific invoice properties to override generic deal data
      invoiceNumber: viewingInvoice.number, 
      issueDate: viewingInvoice.issueDate,
      dueDate: viewingInvoice.dueDate,
      lineItems: viewingInvoice.items,
      ...
      <div className="fixed inset-0 z-50 bg-gray-100 overflow-y-auto">
        <div className="min-h-screen p-4 md:p-8 flex flex-col items-center">
          ...
          <button 
            ...
            className="fixed top-4 right-4 btn btn-circle btn-neutral shadow-lg z-50 print:hidden"
          >
            <XMarkIcon className="w-6 h-6" />
          </button>

          {/* Render the Invoice */}
          <div className="w-full max-w-[210mm] mt-8">
            <InvoicePDF 
              deal={snapshotDeal} 
              user={user} 
            />
            ...
            <div className="card-body p-4 sm:p-5 flex-row items-center justify-between gap-4">
               ...
               <div className="flex items-center gap-3 sm:gap-6">
                 <div className="text-right hidden sm:block">
                   ...
                   <div className="text--[10px] uppercase font-bold opacity-30">Total Amount</div>
                 </div>
