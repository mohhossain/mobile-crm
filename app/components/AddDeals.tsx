...
import { 
  PlusIcon, TrashIcon, BanknotesIcon, TagIcon, UserGroupIcon, 
  ClipboardDocumentListIcon, ArrowRightIcon, ArrowLeftIcon, 
  CheckIcon, ShoppingBagIcon, SparklesIcon
...
// Interfaces
interface ExpenseDraft { description: string; amount: string; category: string; date: string; }
interface Product { id: string; name: string; unitPrice: number; }
interface LineItem { productId?: string; name: string; quantity: number; price: number; }
...
const AddDeals = () => {
  ...
  useEffect(() => {
    ...
    if (clientName && primaryService) setTitle(`${clientName} - ${primaryService}`);
    else if (clientName) setTitle(`${clientName} Project`);
    else if (primaryService) setTitle(`${primaryService} Deal`);
  ...
  const updateLineItem = (index: number, field: keyof LineItem, value: any) => {
    ...
    if (field === 'price' || field === 'quantity') value = Math.max(0, Number(value));
    if (field === 'name') newItems[index].productId = undefined; // Reset product link if manually typed
  ...
  const handleProductSelect = (index: number, productId: string) => {
    ...
    if (product) {
      newItems[index] = { 
        ...newItems[index], 
        productId: product.id, // Explicitly link product
        name: product.name, 
    ...
    <div className="flex flex-col h-full min-h-[400px]">
      ...
      <div className="px-4 py-4 bg-base-100 border-b border-base-200">
        <div className="flex justify-between items-center relative">
          ...
          <div className="absolute top-1/2 left-0 h-0.5 bg-primary -z-10 transition-all duration-300 ease-out" style={{ width: `${((currentStep - 1) / (STEPS.length - 1)) * 100}%` }}></div>
          {STEPS.map((step) => {
            ...
            return (
              <div key={step.id} className="flex flex-col items-center gap-1 group cursor-default">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ring-4 ring-base-100 z-10 ${isActive ? 'bg-primary text-primary-content scale-110 shadow-lg' : isCompleted ? 'bg-primary text-primary-content' : 'bg-base-200 text-base-content/30'}`}>
                  <Icon className="w-4 h-4" />
                </div>
                <span className={`text-[10px] font-bold uppercase tracking-wider bg-base-100 px-1 ${isActive ? 'text-primary' : isCompleted ? 'text-base-content/60' : 'text-base-content/30'} ${isActive ? 'block' : 'hidden sm:block'}`}>
                  {step.title}
                </span>
              </div>
            );
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-base-100">
        <div className="space-y-6">
          ...
          {currentStep === 1 && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                ...
                <ContactMultiSelect contacts={contactOptions} selected={selectedContacts} onChange={setSelectedContacts} />
                <div className="divider text-xs text-base-content/40">OR</div>
                <button type="button" className="btn btn-outline btn-block border-dashed" onClick={() => setShowAddContactModal(true)}>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {lineItems.map((item, idx) => (
                    <div key={idx} className="bg-base-200/40 p-4 rounded-xl border border-base-200 relative group">
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                        <div className="md:col-span-12 lg:col-span-6 flex flex-col gap-2 w-full">
                             ...
                             <select 
                               className="select select-sm select-bordered w-full rounded-md bg-base-100 text-base-content"
                             ...
                             <input className="input input-sm input-bordered w-full rounded-md font-medium" placeholder="Or type custom name..." value={item.name} onChange={(e) => updateLineItem(idx, 'name', e.target.value)} />
                        </div>
                        <div className="md:col-span-12 lg:col-span-6 flex gap-3 w-full">
                          <div className="flex-1">
                             ...
                             <input type="number" min="0" className="input input-sm input-bordered w-full text-center" value={item.quantity} onChange={(e) => updateLineItem(idx, 'quantity', e.target.value)} />
                          </div>
                          <div className="flex-[2]">
                             ...
                             <div className="relative">
                               ...
                               <input type="number" min="0" className="input input-sm input-bordered w-full text-right pl-5" value={item.price} onChange={(e) => updateLineItem(idx, 'price', e.target.value)} />
                             </div>
                          </div>
                ...
                <button onClick={handleAddLineItem} className="btn btn-sm btn-outline border-dashed w-full">+ Add Line Item</button>
              </div>
            </div>
          )}
                  ...
                  <div className="flex flex-col gap-4 animate-in fade-in bg-base-100 p-4 rounded-lg border border-base-200 shadow-sm">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="form-control">
                          ...
                          <input className="input input-sm input-bordered w-full rounded-md" placeholder="e.g. Flight" value={newExpense.description} onChange={e => setNewExpense({...newExpense, description: e.target.value})} />
                        </div>
                        <div className="form-control">
          ...
          {currentStep === 4 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
              <div className="form-control w-full relative">
                ...
                <input type="text" value={title} onChange={(e) => { setTitle(e.target.value); setIsTitleManuallyEdited(true); }} placeholder="e.g. Website Redesign" className="input input-lg input-bordered w-full text-xl font-semibold placeholder:font-normal rounded-md" autoFocus />
              </div>
        ...
        <div className="flex justify-between items-center pt-8 pb-20 sm:pb-4 border-t border-base-200 mt-6">
          {currentStep > 1 ? (
            <button onClick={handleBack} className="btn btn-ghost gap-2 rounded-md"><ArrowLeftIcon className="w-4 h-4" /> Back</button>
          ) : <div></div>}
          {currentStep < 5 ? (
            <button onClick={handleNext} className="btn btn-primary px-8 gap-2 rounded-md">Next <ArrowRightIcon className="w-4 h-4" /></button>
          ) : (
      ...
      {showAddContactModal && (
        <dialog open className="modal modal-bottom sm:modal-middle bg-black/60 backdrop-blur-sm z-[60]">
          <div className="modal-box rounded-xl">
            ...
            <AddLeads onSuccess={(newContact: any) => {
